"""
Stage 1 ablation: Policy BC with zeroed semantic components + WM without semantic.
Usage: python train_stage1_ablation.py --variant no_task
"""
import argparse
import os
import sys
...
def _apply_ablation(task_embs, h_embs, f_embs, ablate_cfg):
    ...
    return task_embs, h_embs, f_embs
...
def train_stage1(args):
    ...
    logdir = os.path.join(args.logdir_base, args.variant, 'stage1')
    os.makedirs(logdir, exist_ok=True)

    print(f"[Stage1-Ablation] Variant: {args.variant}")
    print(f"[Stage1-Ablation] Ablate config: {ablate_cfg}")
    print(f"[Stage1-Ablation] Loading v3 semantic datasets...")
    print(f"[Stage1-Ablation] Policy: WITH ablation, WorldModel: WITHOUT semantic")
    ...
    print(f"[Stage1-Ablation] Train: {len(train_dataset)} trajectories")
    ...
    print(f"[Stage1-Ablation] pi params={n_params_pi:,}, O params={n_params_O:,}")
    print(f"[Stage1-Ablation] Starting: {epochs} epochs, save every {save_interval_epochs} epochs")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, 1):
            ...
            task_embs, h_embs, f_embs = _apply_ablation(task_embs, h_embs, f_embs, ablate_cfg)
            ...
            if batch_idx % log_interval_steps == 0:
                print(f"[Stage1-{args.variant}] Epoch {epoch}/{epochs}, Batch {batch_idx}/{len(train_loader)}  "
                      f"pi={running_pi/log_count:.4f}  O={running_O/log_count:.4f}")
                running_pi = running_O = 0.0
                log_count = 0

        print(f"[Stage1-{args.variant}] Epoch {epoch}/{epochs} complete (global_step={global_step})")
        ...
        if epoch % save_interval_epochs == 0 or epoch == epochs:
            ...
            print(f"[Stage1-{args.variant}] Saved checkpoint at epoch {epoch} -> {ckpt_dir}")

    print(f"[Stage1-{args.variant}] Training complete. Final checkpoint in {logdir}")
