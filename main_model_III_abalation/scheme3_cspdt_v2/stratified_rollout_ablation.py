"""
10-step Rollout evaluation for Policy semantic ablation variants.
  - Policy: SeMDT with zeroed semantic components per ablation config
  - WorldModel: WITHOUT semantic (use_semantic=False)

Usage: python stratified_rollout_ablation.py --variant no_task --checkpoint ... --data ... --output ...
"""
import os
...
def evaluate_ablation(checkpoint_path, data_path, output_path, variant):
    ablate_cfg = ABLATE_VARIANTS[variant]
    print(f"[Ablation Eval] Variant: {variant}")
    print(f"[Ablation Eval] Ablate config: {ablate_cfg}")
    print(f"[Ablation Eval] Policy: ablated semantics, WorldModel: WITHOUT semantic")

    need_encoder = not (ablate_cfg.get('task', True) and ablate_cfg.get('hindsight', True) and ablate_cfg.get('foresight', True))
    if need_encoder:
        ...
        encoder = None
        print("All components ablated, skipping Qwen encoder.")
    ...
    print(f"[Ablation Eval] Test trajectories: {len(trajectories)}")
        ...
        for step in range(min(ROLL_STEPS, len(states) - 1)):
            if step == 0:
                ...
                if encoder is not None:
                    ...
                    task_emb = torch.zeros(1, 1, C.LANGUAGE_EMB_DIM).to(device)
                    h_emb = torch.zeros(1, 1, C.LANGUAGE_EMB_DIM).to(device)
                    f_emb = torch.zeros(1, 1, C.LANGUAGE_EMB_DIM).to(device)

                if encoder is not None:
                    prev_state_np = curr_state_np

            task_emb, h_emb, f_emb = _apply_ablation(task_emb, h_emb, f_emb, ablate_cfg)
    ...
    print(f"\n[Ablation-{variant}] 10-step Rollout Results:")
    ...
    print(f"[Ablation-{variant}] Results saved to: {output_path}")
...
if __name__ == '__main__':
    ...
    evaluate_ablation(args.checkpoint, args.data, args.output, args.variant)
