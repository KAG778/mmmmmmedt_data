"""
10-step Rollout Evaluation — Channel Ablation variant.
  - Policy: SeMDT with channel-masked semantic embeddings
  - WorldModel: WITH semantic but masked channels zeroed
  - sigma=2.0

Usage:
  python stratified_rollout.py --variant A1_no_task --checkpoint <path> --data <path> --output <path>
"""
import os
...
import config_no_sem_sigma2 as C
...
def evaluate_channel_ablation(checkpoint_path, data_path, output_path, variant):
    active = get_active_channels(variant)
    tag = variant_tag(variant)

    print(f"{tag} Model type: {C.MODEL_TYPE}, block_size: {C.BLOCK_SIZE}")
    print(f"{tag} Active channels: {active}")

    # Only load Qwen encoder if at least one channel is active
    if any(active.values()):
        ...
        encoder = None
        print(f"{tag} No Qwen encoder needed — all channels masked")
        ...
        for step in range(min(ROLL_STEPS, len(states) - 1)):
            ...
            if step == 0:
                ...
                if encoder is not None:
                    ...
                    task_emb = torch.zeros(1, 1, 896, device=device)
                    h_emb = torch.zeros(1, 1, 896, device=device)
                    f_emb = torch.zeros(1, 1, 896, device=device)

                prev_state_np = curr_state_np

            # Apply channel masking for policy
            task_m, h_m, f_m = mask_embeddings(
                task_emb.squeeze(0), h_emb.squeeze(0), f_emb.squeeze(0), variant)
            task_m = task_m.unsqueeze(0)
            h_m = h_m.unsqueeze(0)
            f_m = f_m.unsqueeze(0)

            # Policy: get action logits (with masked semantic)
            with torch.no_grad():
                logits, _, _ = policy(
                    states=cur_states,
                    actions=cur_actions,
                    ...
                    task_embeddings=task_m,
                    hindsight_embeddings=h_m,
                    foresight_embeddings=f_m,
                )
                pred_action = logits[:, -1, :].argmax(dim=-1).item()

            # World model: predict with masked semantic concat
            s_t = cur_states[0, -1:, :]
            pred_a_t = torch.tensor([pred_action], device=device)

            # Build masked semantic concat for world model
            sem_concat = torch.cat([
                task_m[0, -1:, :], h_m[0, -1:, :], f_m[0, -1:, :]
            ], dim=-1)  # (1, 2688)

            with torch.no_grad():
                mu, sigma, delta_saps2 = world_model.predict(
                    s_t, pred_a_t, semantic=sem_concat, mc_samples=C.MC_SAMPLES)
    ...
    print(f"\n{tag} Channel Ablation ({variant}) 10-step Rollout Results:")
...
if __name__ == '__main__':
    ...
    evaluate_channel_ablation(args.checkpoint, args.data, args.output, args.variant)
