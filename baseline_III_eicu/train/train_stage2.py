...
def search_better_action(policy, world_model, state, current_action, search_horizon=10,
    ...
    # Normalize current_action to discrete index
    # current_action can be: LongTensor scalar/index, or one-hot float tensor
    if current_action.dtype in (torch.long, torch.int64, torch.int32):
        current_action_idx = current_action.item() if current_action.dim() == 0 else current_action[0].item()
        n_actions = world_model.action_dim
    else:
        current_action_idx = torch.argmax(current_action).item()
        n_actions = current_action.shape[-1]
    ...
    with torch.no_grad():
        doc_mu, doc_sigma, doc_delta = world_model.predict_with_uncertainty(
            state_2d, current_action, semantic_tensor, n_samples=mc_samples)
    ...
    for action_idx in candidates:
        # Pass discrete index directly; world_model.forward handles one-hot
        action_tensor = torch.tensor([action_idx], dtype=torch.long, device=device)

        with torch.no_grad():
            mu, sigma, delta = world_model.predict_with_uncertainty(
                state_2d, action_tensor, semantic_tensor, n_samples=mc_samples)
    ...
    # Return one-hot encoded best action for downstream MSE/Q-value losses
    best_action_onehot = torch.zeros(1, n_actions, device=device)
    best_action_onehot[0, best_action_idx] = 1.0
...
def train_stage2(config_path, checkpoint_path, output_dir=None):
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs - 1 and not (output_dir / 'best_checkpoint.pt').exists():
            torch.save({
                'epoch': epoch,
                'policy_state_dict': policy.state_dict(),
                'world_model_state_dict': world_model.state_dict(),
            ...
            print(f"  -> Saved last-epoch checkpoint as fallback (advantage={avg_advantage:.4f})")
...
def train_stage2_with_semantic(config_path, checkpoint_path, output_dir=None):
    """
