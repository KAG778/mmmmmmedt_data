...
class TrajectoryDataset(Dataset):
    ...
    def __init__(self, data_path, max_length=20, action_dim=4, stratified_sampling=True, low_oversample=3, normalize_states=True):
        ...
        for idx in self.indices:
            ...
            if 'acuities' in traj and traj['acuities'] is not None:
                ...
                init_saps2 = 40.0  # eICU: default to low (<45)
        ...
        if stratified_sampling:
            ...
            print(f"  Low (<45):   {len(low_indices):5d} -> oversampled by {low_oversample}x")
    ...
    def __getitem__(self, idx):
        ...
        states_full = traj['dem_observations']  # (T, 45)

        # Truncate to max_length for DT compatibility (eICU trajectories can be very long)
        if len(states_full) > self.max_length:
            start = np.random.randint(0, len(states_full) - self.max_length + 1)
            states_full = states_full[start:start + self.max_length]
            actions_full = traj['actions'][start:start + self.max_length]
            if 'acuities' in traj and traj['acuities'] is not None:
                acuities_full = traj['acuities'][start:start + self.max_length]
            else:
                acuities_full = None
        else:
            actions_full = traj['actions']
            acuities_full = traj.get('acuities', None)
        ...
        actions = torch.LongTensor(actions_full)

        # Compute reward from SAPS2 delta: reward = -delta (improvement = positive reward)
        if acuities_full is not None:
            acuities_saps2 = acuities_full[:, 2]
            saps2_delta = acuities_saps2[1:] - acuities_saps2[:-1]
            rewards = -saps2_delta
        else:
