...
"""
10步 Rollout 评估 — NoSemPolicy: Policy WITHOUT semantic (DT), WM WITH semantic
  Step 0: 预计算的 v3 嵌入 (从 pickle 数据文件)
  Step 1+: v7 在线生成嵌入 (仅基于 World Model 预测的 45 维 state)
  Policy (DT): 忽略语义嵌入但需要合法张量
  World Model: 使用语义嵌入
"""
import sys
import pickle
...
def evaluate_no_sem_policy(checkpoint_path, data_path, output_path, sigma_threshold=2.0):
    print(f"[NoSemPolicy Eval] Model type: {C.MODEL_TYPE}, block_size: {C.BLOCK_SIZE}")
    print(f"[NoSemPolicy Eval] Policy: DT (no semantic), WorldModel: WITH semantic")
    print(f"[NoSemPolicy Eval] Sigma threshold: {sigma_threshold}")

    # Load encoder for v7 prompts (WM needs semantic from step 1+)
    ...
    for idx, traj in enumerate(trajectories):
        ...
        for step in range(min(ROLL_STEPS, len(states) - 1)):
            # --- Get semantic embeddings (for Policy tensor compat + WM semantic) ---
            if step == 0:
            ...
            # --- World model: predict next state + delta_saps2 (WITH semantic) ---
            s_t = cur_states[0, -1:, :]
    ...
    print(f"\nNoSemPolicy (DT) v3+v7 10步Rollout结果:")
...
if __name__ == '__main__':
    ...
    evaluate_no_sem_policy(args.checkpoint, args.data, args.output, args.sigma_threshold)
