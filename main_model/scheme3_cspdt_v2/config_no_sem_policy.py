# CSP-DT Config: Policy WITHOUT semantic (DT model_type), WM WITH semantic
# Diff from config.py: MODEL_TYPE='DT' (3-token), BLOCK_SIZE=CONTEXT_LENGTH*3
...
BLOCK_SIZE = CONTEXT_LENGTH * 3  # DT: 3 tokens per timestep (RTG, state, action)
MODEL_TYPE = 'DT'     # 3-token variant, no semantic embeddings in policy

# World Model (O) — unchanged, still uses semantic
O_HIDDEN = 256
MC_DROPOUT = 0.2
