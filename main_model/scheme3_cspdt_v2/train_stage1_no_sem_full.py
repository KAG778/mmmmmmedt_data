"""
Stage 1: BOTH Policy and WM WITHOUT semantic embeddings.
  - pi: behaviour cloning (CE loss on actions) with continuous SAPS-II RTG (DT model_type)
  - O: NLL state loss + SAPS2 delta MSE loss (WITHOUT semantic)
"""
import argparse
import os
import sys
...
def train_stage1(args):
    ...
    print(f"[Stage1-NoSemFull] Loading v3 semantic datasets (gamma={C.RTG_GAMMA}, scale={C.RTG_SCALE})...")
    ...
    print(f"[Stage1-NoSemFull] Train: {len(train_dataset)} trajectories, Val: {len(val_dataset)} trajectories")
    ...
    print(f"[Stage1-NoSemFull] pi params={n_params_pi:,}, O params={n_params_O:,}")
    print(f"[Stage1-NoSemFull] model_type={C.MODEL_TYPE}, block_size={C.BLOCK_SIZE}")
    print(f"[Stage1-NoSemFull] WM use_semantic=False")
    print(f"[Stage1-NoSemFull] Starting: {epochs} epochs, save every {save_interval_epochs} epochs")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, 1):
            ...
            # --- O: NLL + SAPS2 delta prediction (WITHOUT semantic) ---
            ...
            if batch_idx % log_interval_steps == 0:
                print(f"[Stage1-NoSemFull] Epoch {epoch}/{epochs}, Batch {batch_idx}/{len(train_loader)}  "
        ...
        print(f"[Stage1-NoSemFull] Epoch {epoch}/{epochs} complete (global_step={global_step})")
        print(f"[Stage1-NoSemFull] Validation: pi={avg_val_pi:.4f}, O={avg_val_O:.4f}, total={avg_val_total:.4f}")
        ...
        if epoch % save_interval_epochs == 0 or epoch == epochs:
            ...
            print(f"[Stage1-NoSemFull] Saved checkpoint at epoch {epoch} -> {ckpt_dir}")
        ...
        if avg_val_total < best_val_loss:
            ...
            print(f"[Stage1-NoSemFull] New best checkpoint! Epoch {epoch}, val_loss={avg_val_total:.4f}")

    print(f"[Stage1-NoSemFull] Training complete. Best checkpoint at epoch {best_epoch} with val_loss={best_val_loss:.4f}")
    print(f"[Stage1-NoSemFull] Best checkpoint saved in {os.path.join(args.logdir, 'best_checkpoint')}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--datadir', default=C.DATA_DIR)
    parser.add_argument('--logdir', default='./checkpoints_no_sem_full/stage1')
