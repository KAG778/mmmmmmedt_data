...
ABLATION: Policy WITHOUT semantic (DT model_type), WM WITH semantic.
...
def search_better_action(policy, world_model, state, current_action_idx,
    ...
    WM uses semantic embeddings. Policy (DT) ignores semantic but needs valid tensors.
    """
    device = state.device
...
def train_stage2(args):
    ...
    print(f"[Stage2-NoSemPolicy] Loading v3 semantic datasets...")
    ...
    print(f"[Stage2-NoSemPolicy] Loaded {len(trajectories)} trajectories")
    ...
    print(f"[Stage2-NoSemPolicy] Stratified sampling:")
    ...
    if os.path.exists(norm_path):
        ...
        print(f"[Stage2-NoSemPolicy] Loaded normalization params from {norm_path}")
    else:
        print(f"[Stage2-NoSemPolicy] Warning: normalization params not found")
        state_mean = np.zeros(C.STATE_DIM)
        state_std = np.ones(C.STATE_DIM)

    # Initialize semantic generator for better_action embeddings (WM uses semantic)
    ...
    print(f"[Stage2-NoSemPolicy] Initialized semantic generator for WM semantic input")
    ...
    print(f"[Stage2-NoSemPolicy] Starting: {epochs} epochs x {selfplay_iterations} iterations, "
    ...
    for epoch in range(epochs):
        ...
        for iteration in range(selfplay_iterations):
            ...
            if advantage > 0:
                ...
                # Generate better_action's embeddings (WM needs semantic)
                ...
                # World model loss: confidence-gated semi-bootstrap (WITH semantic)
        ...
        print(f"[Stage2-NoSemPolicy] Epoch {epoch+1}/{epochs} (global_step={global_step}): "
    ...
    print(f"\n[Stage2-NoSemPolicy] Training complete. Best advantage: {best_advantage:.4f} "
          f"(global_steps={global_step})")
    print(f"[Stage2-NoSemPolicy] Checkpoints in {args.logdir}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--datadir', default=C.DATA_DIR)
    parser.add_argument('--logdir', default='./checkpoints_no_sem_policy/stage2')
    parser.add_argument('--policy_ckpt', default='./checkpoints_no_sem_policy/stage1/best_checkpoint/policy.pt')
    parser.add_argument('--world_model_ckpt',
                        default='./checkpoints_no_sem_policy/stage1/best_checkpoint/world_model.pt')
