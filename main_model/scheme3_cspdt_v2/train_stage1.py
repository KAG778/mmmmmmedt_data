"""
Stage 1: Cold-start training with v3 semantic embeddings.
  - pi: behaviour cloning (CE loss on actions) with continuous SAPS-II RTG
  - O: NLL state loss + SAPS2 delta MSE loss
"""
import argparse
import os
import sys
...
def train_stage1(args):
    ...
    print(f"[Stage1] Loading v3 semantic datasets (gamma={C.RTG_GAMMA}, scale={C.RTG_SCALE})...")
    ...
    print(f"[Stage1] Train: {len(train_dataset)} trajectories, Val: {len(val_dataset)} trajectories")

    # Verify RTG is continuous
    rtg_vals = train_dataset.trajectories[0]['returns_to_go']
    print(f"[Stage1] Sample RTG (traj 0): {rtg_vals[:5]}")
    ...
    print(f"[Stage1] pi params={n_params_pi:,}, O params={n_params_O:,}")
    print(f"[Stage1] Starting: {epochs} epochs, save every {save_interval_epochs} epochs")
    print(f"[Stage1] Training set size: {len(train_dataset)} trajectories")
    ...
    if hasattr(args, 'resume_from') and args.resume_from:
        resume_dir = os.path.join(args.logdir, f"epoch_{args.resume_from}")
        if os.path.exists(resume_dir):
            policy.load_state_dict(torch.load(os.path.join(resume_dir, "policy.pt"), map_location=device, weights_only=True))
            world_model.load_state_dict(torch.load(os.path.join(resume_dir, "world_model.pt"), map_location=device, weights_only=True))
            start_epoch = args.resume_from + 1
            global_step = start_epoch * len(train_loader)
            print(f"[Stage1] Resumed from epoch {args.resume_from}, continuing from epoch {start_epoch}")
            # Restore best_val_loss from metadata if available
            best_meta = os.path.join(args.logdir, "best_checkpoint", "metadata.json")
            if os.path.exists(best_meta):
                ...
                print(f"[Stage1] Best so far: epoch {best_epoch}, val_loss={best_val_loss:.4f}")
        else:
            print(f"[Stage1] WARNING: resume checkpoint not found at {resume_dir}, starting fresh")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, 1):
            ...
            if mask_flat.sum() > 0:
                ...
                # SAPS2 delta loss: predict delta_saps2 = saps2[t+1] - saps2[t]
            ...
            if batch_idx % log_interval_steps == 0:
                print(f"[Stage1] Epoch {epoch}/{epochs}, Batch {batch_idx}/{len(train_loader)}  "
        ...
        print(f"[Stage1] Epoch {epoch}/{epochs} complete (global_step={global_step})")
        print(f"[Stage1] Validation: pi={avg_val_pi:.4f}, O={avg_val_O:.4f}, total={avg_val_total:.4f}")
        ...
        if epoch % save_interval_epochs == 0 or epoch == epochs:
            ...
            print(f"[Stage1] Saved checkpoint at epoch {epoch} -> {ckpt_dir}")
        ...
        if avg_val_total < best_val_loss:
            ...
            print(f"[Stage1] ✓ New best checkpoint! Epoch {epoch}, val_loss={avg_val_total:.4f}")

    print(f"[Stage1] Training complete. Best checkpoint at epoch {best_epoch} with val_loss={best_val_loss:.4f}")
    print(f"[Stage1] Best checkpoint saved in {os.path.join(args.logdir, 'best_checkpoint')}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--datadir', default=C.DATA_DIR)
    parser.add_argument('--logdir', default='./checkpoints/stage1')
