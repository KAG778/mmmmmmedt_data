...
"""
10步 Rollout 评估 — NoSemFull: BOTH Policy and WM WITHOUT semantic
  Policy (DT): 忽略语义嵌入，使用预计算嵌入或零张量
  World Model: 不使用语义（semantic=None）
  无 Qwen 编码器加载（评估更快）
"""
import sys
import pickle
...
def evaluate_no_sem_full(checkpoint_path, data_path, output_path, sigma_threshold=2.0):
    print(f"[NoSemFull Eval] Model type: {C.MODEL_TYPE}, block_size: {C.BLOCK_SIZE}")
    print(f"[NoSemFull Eval] Policy: DT (no semantic), WorldModel: WITHOUT semantic")
    print(f"[NoSemFull Eval] No Qwen encoder needed — fast evaluation")
    print(f"[NoSemFull Eval] Sigma threshold: {sigma_threshold}")

    # Build models (no encoder needed)
    policy = build_policy(
    ...
    # Zero tensors for DT branch (DT ignores values, just needs valid shape)
    zero_emb = torch.zeros(1, 1, C.LANGUAGE_EMB_DIM, device=device)
    ...
    for idx, traj in enumerate(trajectories):
        ...
        for step in range(min(ROLL_STEPS, len(states) - 1)):
            # --- Get embeddings for policy (DT ignores but needs valid tensors) ---
            if step == 0:
                # Step 0: use precomputed embeddings from pickle
                ...
                # Step 1+: zero tensors (DT completely ignores, value doesn't matter)
                task_emb = zero_emb
                h_emb = zero_emb
                f_emb = zero_emb

            # --- Policy: get action logits (DT branch ignores semantic) ---
            with torch.no_grad():
                logits, _, _ = policy(
                    states=cur_states,
                    actions=cur_actions,
            ...
            with torch.no_grad():
                mu, sigma, delta_saps2 = world_model.predict(s_t, pred_a_t, semantic=None, mc_samples=C.MC_SAMPLES)
    ...
    print(f"\nNoSemFull (DT) 10步Rollout结果 (no semantic at all):")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, required=True,
                        help='Checkpoint dir or single file')
    parser.add_argument('--data', type=str, required=True,
                        help='Test pickle file')
    parser.add_argument('--output', type=str, required=True,
                        help='Output JSON path')
    parser.add_argument('--sigma_threshold', type=float, default=2.0,
                        help='Sigma threshold for filtering (default: 2.0)')
    args = parser.parse_args()

    evaluate_no_sem_full(args.checkpoint, args.data, args.output, args.sigma_threshold)
