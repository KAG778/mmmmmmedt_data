...
ABLATION: Policy uses semantic, World Model does NOT use semantic.
...
def train_stage2(args):
    ...
    # Pre-normalize states in trajectories (V3SemanticDataset only normalizes in __getitem__,
    # but Stage2 accesses trajectories directly)
    if train_dataset.normalize_states and hasattr(train_dataset, 'state_mean'):
        print(f"[Stage2] Pre-normalizing dem_observations in trajectories...")
        for traj in trajectories:
            traj['dem_observations'] = (traj['dem_observations'] - train_dataset.state_mean) / train_dataset.state_std
        print(f"[Stage2] Normalized {len(trajectories)} trajectories")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--policy_ckpt', default='./checkpoints/stage1/step_200000/policy.pt')
    parser.add_argument('--world_model_ckpt',
                        default='./checkpoints/stage1/step_200000/world_model.pt')
