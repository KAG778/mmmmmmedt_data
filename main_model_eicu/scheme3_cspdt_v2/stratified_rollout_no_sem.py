...
def stratify(initial_saps2):
    ...
    if initial_saps2 < 45: return 'low'
    elif initial_saps2 < 55: return 'mid'
    else: return 'high'
...
def _resolve_ckpt(checkpoint_path, device):
    # Check directory format first (policy.pt + world_model.pt)
    if os.path.isdir(checkpoint_path):
        policy_sd = torch.load(os.path.join(checkpoint_path, 'policy.pt'), map_location=device, weights_only=False)
        world_sd = torch.load(os.path.join(checkpoint_path, 'world_model.pt'), map_location=device, weights_only=False)
