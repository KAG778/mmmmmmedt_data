"""
Stage 2 (Epoch-based, No WM Semantic): Counterfactual Self-Play.
  - pi: behaviour cloning with semantic embeddings — UNCHANGED
  - O: semi-bootstrap world model — WITHOUT semantic embeddings
  ...
  - Confidence-gated semi-bootstrap
...
def train_stage2(args):
    ...
    print(f"[Stage2-NoSemWM-Epoch] Loading v3 semantic datasets...")
    print(f"[Stage2-NoSemWM-Epoch] Policy: WITH semantic, WorldModel: WITHOUT semantic")
    ...
    print(f"[Stage2-NoSemWM-Epoch] Loaded {len(trajectories)} trajectories")
    ...
    for i, traj in enumerate(trajectories):
        ...
        # eICU thresholds: SAPS2 range 11-81, mean=51.4
        if init_saps2 < 45:
    ...
    print(f"[Stage2-NoSemWM-Epoch] Stratified sampling:")
    print(f"  Low (<45):   {len(low_indices):5d}")
    print(f"  Mid (45-55): {len(mid_indices):5d}")
    print(f"  High (>=55): {len(high_indices):5d}")
    ...
    if os.path.exists(norm_path):
        ...
        print(f"[Stage2-NoSemWM-Epoch] Loaded normalization params from {norm_path}")
    else:
        print(f"[Stage2-NoSemWM-Epoch] Warning: normalization params not found")
        state_mean = np.zeros(C.STATE_DIM)
        state_std = np.ones(C.STATE_DIM)

    # Initialize semantic generator
    text_encoder = PromptTextEncoder(
        model_name="/home/wangmeiyi/models/Qwen2.5-0.5B-Instruct",
        device=device,
        max_length=256,
    )
    semantic_generator = SemanticGenerator(text_encoder, device=device)
    print(f"[Stage2-NoSemWM-Epoch] Initialized semantic generator")
    ...
    print(f"[Stage2-NoSemWM-Epoch] Starting: {epochs} epochs x {selfplay_iterations} iterations, "
    ...
    for epoch in range(epochs):
        ...
        for iteration in range(selfplay_iterations):
            ...
            if advantage > 0:
                ...
                forward_kwargs = dict(
                    ...
                    hindsight_embeddings=better_h_emb.unsqueeze(0),
                    foresight_embeddings=better_f_emb.unsqueeze(0),
                )
                if use_atg:
        ...
        # Save periodic checkpoint as directory (compatible with Stage1 format)
        if (epoch + 1) % args.save_interval == 0 or (epoch + 1) == epochs:
            ckpt_dir = os.path.join(args.logdir, f"epoch_{epoch+1}")
    ...
    print(f"\n[Stage2-NoSemWM-Epoch] Training complete. Best advantage: {best_advantage:.4f} "
          f"(global_steps={global_step})")
    print(f"[Stage2-NoSemWM-Epoch] Checkpoints in {args.logdir}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--datadir', default=C.DATA_DIR)
    parser.add_argument('--logdir', default='./checkpoints_no_sem_wm/stage2')
    parser.add_argument('--policy_ckpt', required=True)
    parser.add_argument('--world_model_ckpt', required=True)
