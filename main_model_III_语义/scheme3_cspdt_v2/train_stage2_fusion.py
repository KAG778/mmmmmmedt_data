"""
Stage 2 (Fusion Ablation): Counterfactual Self-Play with fused semantic-numeric policy.
  - Policy: SeMDT_Concat/Residual/Gated/CrossAttn (3-token)
  - WorldModel: NO semantic (use_semantic=False)
  - sigma=2.0, search_radius=±2

Usage:
  python train_stage2_fusion.py --fusion concat \
    --policy_ckpt checkpoints_fusion/concat/stage1/epoch_100/policy.pt \
    --world_model_ckpt checkpoints_fusion/concat/stage1/epoch_100/world_model.pt
"""
import argparse
import os
import sys
...
def search_better_action(policy, world_model, state, current_action_idx,
    ...
    """Search for a better action using world model predictions.
    No channel masking needed — fusion combines all semantic channels."""
    device = state.device

    # Doctor action's predicted delta (no semantic for WM)
    with torch.no_grad():
        doc_action_t = torch.tensor([current_action_idx], device=device)
        doc_mu, doc_sigma, doctor_delta = world_model.predict(
            state, doc_action_t, mc_samples=mc_samples)
    ...
    # Policy's suggested action (fused semantic)
    with torch.no_grad():
    ...
    for action_idx in candidates:
        ...
        with torch.no_grad():
            mu_cand, sigma_cand, candidate_delta = world_model.predict(
                state, action_t, mc_samples=mc_samples)
...
def train_stage2(args):
    ...
    model_type, block_size, tokens = C.configure_variant(args.fusion)
    tag = f"[{args.fusion}]"
    ...
    ckpt_dir, log_dir, _ = C.get_dirs(args.fusion)
    logdir = args.logdir or os.path.join(ckpt_dir, 'stage2')
    os.makedirs(logdir, exist_ok=True)

    print(f"{tag} Stage 2 Fusion Ablation Training")
    ...
    for epoch in range(epochs):
        ...
        for iteration in range(selfplay_iterations):
            ...
            # Search (no channel masking — fusion combines all)
            ...
            if advantage > 0:
                ...
                with torch.no_grad():
                    ...
                    _, _, pred_delta = world_model.predict(
                        state_t, better_a_t, mc_samples=C.MC_SAMPLES)

                # Policy loss (fused semantic, no masking)
                # NOTE: pass actions=None for fusion 3-token models.
                # With T=1 and targets=None, passing actions causes the model
                # to remove the only action token, making logits empty at 2::3.
                # actions=None uses 2-token [fused, RTG] scheme which works correctly.
