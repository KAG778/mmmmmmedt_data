"""
Stage 1 (Fusion Ablation): Cold-start training with fused semantic-numeric policy.
...
Usage:
  python train_stage1_fusion.py --fusion concat
  python train_stage1_fusion.py --fusion gated --epochs 100
"""
import argparse
import os
import sys
...
def train_stage1(args):
    ...
    logdir = args.logdir or os.path.join(ckpt_dir, 'stage1')
    os.makedirs(logdir, exist_ok=True)

    print(f"{tag} Stage 1 Fusion Ablation Training")
    ...
    # Policy with fusion architecture (3-token)
    policy = build_policy(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, 1):
            ...
            # --- pi: behaviour cloning (fusion policy, no channel masking) ---
            ...
            if mask_flat.sum() > 0:
                ...
                nll = world_model.nll_loss(s_t_m, a_t_m, s_next_m)

                saps2_target = delta_saps2[:, :-1, :].reshape(-1, 1)[mask_flat]
                saps2_loss = world_model.saps2_loss(s_t_m, a_t_m, saps2_target)
        ...
        if epoch % save_interval_epochs == 0 or epoch == epochs:
            ckpt_subdir = os.path.join(logdir, f"epoch_{epoch}")
            os.makedirs(ckpt_subdir, exist_ok=True)
            torch.save(policy.state_dict(), os.path.join(ckpt_subdir, "policy.pt"))
            torch.save(world_model.state_dict(), os.path.join(ckpt_subdir, "world_model.pt"))
            print(f"{tag} Saved checkpoint at epoch {epoch} -> {ckpt_subdir}")
