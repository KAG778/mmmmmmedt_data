...
class MeDT(GPT):
    ...
    def forward(
        ...
        # ===== Fusion variants: 3 tokens [fused, RTG, action] =====
        elif self.model_type in ('SeMDT_Concat', 'SeMDT_Residual', 'SeMDT_Gated', 'SeMDT_CrossAttn'):
            if task_embeddings is None or hindsight_embeddings is None or foresight_embeddings is None:
                raise ValueError(f"{self.model_type} requires task/hindsight/foresight embeddings.")
            # Ensure semantic T matches states T (rollout may pass T_sem=1 with T>1)
            T_states = states.shape[1]
            te_f = task_embeddings.type(torch.float32)
            he_f = hindsight_embeddings.type(torch.float32)
            fe_f = foresight_embeddings.type(torch.float32)
            if te_f.shape[1] != T_states:
                te_f = te_f.expand(-1, T_states, -1)
                he_f = he_f.expand(-1, T_states, -1)
                fe_f = fe_f.expand(-1, T_states, -1)
            sem_raw = torch.cat([te_f, he_f, fe_f], dim=-1)  # (B, T, 2688)

            if self.model_type == 'SeMDT_Concat':
                fused = self.concat_proj(torch.cat([states.type(torch.float32), sem_raw], dim=-1))
            elif self.model_type == 'SeMDT_Residual':
                fused = state_embeddings + self.residual_sem_proj(sem_raw)
            elif self.model_type == 'SeMDT_Gated':
                gate = self.gate_proj(torch.cat([states.type(torch.float32), sem_raw], dim=-1))
                fused = gate * state_embeddings + (1 - gate) * self.gate_sem_proj(sem_raw)
            elif self.model_type == 'SeMDT_CrossAttn':
                sem_tokens = torch.stack([
                    self.task_text_emb(te_f),
                    self.h_text_emb(he_f),
                    self.f_text_emb(fe_f),
                ], dim=2)  # (B, T, 3, n_embd)
                B, T, _, D = sem_tokens.shape
                flat_state = state_embeddings.reshape(B*T, 1, D)
                flat_sem = sem_tokens.reshape(B*T, 3, D)
                attn_out, _ = self.cross_attn(flat_state, flat_sem, flat_sem)
                fused = (state_embeddings + self.cross_norm(attn_out.reshape(B, T, D)))
            ...
            if actions is not None:
                ...
                n_tok = 3
                token_embeddings = torch.zeros(
                    (states.shape[0], states.shape[1]*n_tok - int(targets is None), self.config.n_embd),
                    dtype=torch.float32, device=state_embeddings.device)
                token_embeddings[:, ::n_tok, :] = fused
                token_embeddings[:, 1::n_tok, :] = rtg_embeddings
                token_embeddings[:, 2::n_tok, :] = action_embeddings[:, -states.shape[1] + int(targets is None):, :]
            else:
                n_tok = 2
                token_embeddings = torch.zeros(
                    (states.shape[0], states.shape[1]*n_tok, self.config.n_embd),
                    dtype=torch.float32, device=state_embeddings.device)
                token_embeddings[:, ::n_tok, :] = fused
                token_embeddings[:, 1::n_tok, :] = rtg_embeddings
            ...
            my_pos_emb = torch.zeros(
                timesteps.shape[0],
                token_embeddings.shape[1],
                self.config.n_embd, device=token_device)
            if actions is not None:
                # 3 tokens: fused(T), RTG(T), action(T-1)
                my_pos_emb[:, ::3, :] = timesteps
                my_pos_emb[:, 1::3, :] = timesteps
                my_pos_emb[:, 2::3, :] = timesteps[:, -states.shape[1] + int(targets is None):, :]
            else:
                # 2 tokens: fused(T), RTG(T)
                my_pos_emb[:, ::2, :] = timesteps
                my_pos_emb[:, 1::2, :] = timesteps
        ...
        if actions is not None and self.model_type == 'DT':
            ...
            logits = logits
        # Fusion variants: action logits from slot 2 (of 3 tokens: fused, RTG, action)
        elif actions is not None and self.model_type in ('SeMDT_Concat', 'SeMDT_Residual', 'SeMDT_Gated', 'SeMDT_CrossAttn'):
            logits = logits[:, 2::3, :]
        elif actions is None and self.model_type in ('SeMDT_Concat', 'SeMDT_Residual', 'SeMDT_Gated', 'SeMDT_CrossAttn'):
            logits = logits[:, 0::2, :]
        else:
            raise NotImplementedError()
