"""
Generate v3 embeddings for eICU phys45 data.
Input:  other_data/eicu_phys45_deliver/{train,val,test}_Phys45.pickle
Output: data/eicu_v3/{train,val,test}_Phys45_v3.pickle + normalization_params.pkl
"""
import sys
import os
import pickle
...
def compute_normalization(trajectories):
    ...
    return {
        "state_mean": np.nanmean(all_states, axis=0).astype(np.float32),
        "state_std": np.nanstd(all_states, axis=0).astype(np.float32),
    }


def main():
    input_dir = "/home/wangmeiyi/AuctionNet/medical/last_exp/other_data/eicu_phys45_deliver"
    output_dir = "/home/wangmeiyi/AuctionNet/medical/last_exp/data/eicu_v3"
    ...
    for split in [s.strip() for s in splits.split(",")]:
        ...
        # Skip if already generated and loadable
        if os.path.exists(output_path):
            print(f"\n[INFO] === Checking existing {split} ===")
            try:
                with open(output_path, "rb") as f:
                    _d = pickle.load(f)
                print(f"[INFO] {split} already generated ({len(_d)} trajs), skipping")
                del _d
                continue
            except Exception:
                print(f"[INFO] {split} file exists but corrupted, regenerating...")
        ...
        # Remove text prompts before saving to avoid encoding issues
        # They are only needed for debugging, not for training
        for traj in trajectories:
            for key in ["v3_task_prompts", "v3_hindsight_prompts", "v3_foresight_prompts"]:
                traj.pop(key, None)
    ...
    # Compute normalization from raw training set (avoid reloading 24GB v3 pickle)
    print("\n[INFO] Computing normalization params from raw training set...")
    raw_train_path = os.path.join(input_dir, "train_Phys45.pickle")
    with open(raw_train_path, "rb") as f:
        raw_train = pickle.load(f)
    norm_params = compute_normalization(raw_train)
    del raw_train
