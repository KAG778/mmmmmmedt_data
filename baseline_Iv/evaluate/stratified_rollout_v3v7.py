...
def evaluate_baseline_v3v7(config_path, checkpoint_path, data_path, output_path, max_traj=0):
    # Load config
    ...
    # Check if models actually use semantic embeddings
    uses_semantic = (
        policy_cfg.get('semantic_embed') is not None
        or world_cfg.get('semantic_embed') is not None
    )
    ...
    if uses_semantic:
        ...
        print("Models have semantic_embed=null, skipping Qwen encoder (dummy embeddings used).")
        encoder = None
        # Pre-allocate a dummy semantic tensor (will be ignored by models)
        dummy_sem = torch.zeros(1, 2688, device=device)
    ...
    if os.path.exists(norm_path):
        ...
        state_mean = np.zeros(45)
        state_std = np.ones(45)
    ...
    for idx, traj in enumerate(trajectories):
        ...
        states_normalized = (states_raw - state_mean) / state_std
        states = torch.FloatTensor(states_normalized).to(device)
            ...
            if uses_semantic:
                ...
                if step == 0:
                    ...
                    if step == 1:
                        ...
                        prev_state_np = current_state[0].cpu().detach().numpy()
                    ...
                    sem_tensor = torch.cat([embs[0:1], embs[1:2], embs[2:3]], dim=-1).to(device)
            else:
                # Models ignore semantic, use dummy tensor
                sem_tensor = dummy_sem
