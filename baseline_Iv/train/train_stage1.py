...
class TrajectoryDataset(Dataset):
    ...
    def __init__(self, data_path, max_length=20, action_dim=25, stratified_sampling=True, low_oversample=3, normalize_states=True):
        ...
        if stratified_sampling:
            ...
            print(f"  Low (<50):   {len(low_indices):5d} -> oversampled by {low_oversample}x")
    ...
    def __getitem__(self, idx):
        ...
        # Truncate to max_length (context_length)
        T = len(states)
        start = 0
        if T > self.max_length:
            ...
            actions_discrete = traj['actions'][start:end]
        else:
        ...
        # Compute reward from SAPS2 delta on the (truncated) segment
        if 'acuities' in traj and traj['acuities'] is not None:
            acuities = traj['acuities'][:, 2]
            if T > self.max_length:
                acuities = acuities[start:end]
        ...
        rewards_full = np.concatenate([[0], rewards])
...
def train_stage1(config_path, output_dir=None, epochs_override=None):
...
if __name__ == '__main__':
    ...
    train_stage1(args.config, args.output_dir, epochs_override=args.epochs if args.epochs > 0 else None)
